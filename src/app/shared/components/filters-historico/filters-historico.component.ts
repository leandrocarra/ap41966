...
@Component({
    selector: "app-filters-historico",
    templateUrl: "./filters-historico.component.html",
    styleUrls: ["./filters-historico.component.scss"],
    encapsulation: ViewEncapsulation.None
})
export class FiltersHistoricoComponent implements OnInit {
    ...
    filterByStatus(status: any) {
        let filtrados;
        filtrados = this.itens.filter((res: {fatura: FaturaDTO, historico: boolean | LeituraDeConsumoDTO}) => {
            if (this._segundaViaService.getStatus(res.fatura.statusFatura) == status) {
                return res.fatura.statusFatura.toLocaleLowerCase();
            }
        });
    ...
    filterByAno(ano: any) {
        ...
        filtrados = this.itens.filter((res: {fatura: FaturaDTO, historico: boolean | LeituraDeConsumoDTO}) => {
            let data = res.fatura.dataVencimento ?? "";
    ...
    getStatusPossiveis(): Array<string> {
        ...
        this.itens.forEach((element: any) => {
            statusPossiveis.push(this._segundaViaService.getStatus(element.fatura.statusFatura));
        });
