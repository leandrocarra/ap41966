...
export class CalculadoraUtilsService {
	...
	itensCombo: any;
	palavrasArt: Array<string>;
	monofasicoPadrao: any;
	bifasicoPadrao: any;
	trifasicoPadrao: any;
	itensCategoriaApartamento: any;
	...
	constructor() {
		...
		this.itensCategoriaApartamento = {
			'MONOFÁSICA': this.monofasicoPadrao,
			'BIFÁSICA': this.bifasicoPadrao,
			'TRIFÁSICA': this.trifasicoPadrao
		};
	...
	calcular(equipamentos: any): Promise<number> {
		return new Promise((resolve) => {
			...
			if (equipamentos.length > 0) {
				equipamentos.forEach((equipamento: any, index: any) => {
	...
	getCategoria(potencia: number): Promise<string> {
		return new Promise((resolve) => {
			if (potencia <= 10000) {
				resolve('MONOFÁSICA');
