...
@Component({
  selector: 'app-montante-gerador',
  templateUrl: './montante-gerador.component.html',
  styleUrls: ['./montante-gerador.component.scss']
})
export class MontanteGeradorComponent implements OnInit {
  ...
  // DataSets
  contratadoChartDataSet: ChartDataset = {
    label: "Montante de uso contratado fio gerador exportação (kW)",
    data: [],
  ...
  sistemaChartDataSet: ChartDataset = {
    label: "Montante de uso sistema fio gerador exportação (kW)",
    data: [],
  ...
  ngOnInit() {
    this.contratado = this.dataContratado;
    this.sistema = this.dataSistema;
  }
  ...
  ngAfterViewInit(): void {
    ...
    const mesesRotulos = this.getMonthsLabels(this.contratado);
  ...
  initChart() {
    ...
    const max = this.getMaxDataSet(); // usar o dataSet que possui o maior valor numérico
  ...
  fillDataChart(order = false) {
    ...
    let chatData = this.ordenarHistorico(this.contratado);
    let data = this.fillDataChartSet(chatData, order);

    this.contratadoChartDataSet.data = data;
    datasets.push(this.contratadoChartDataSet);

    chatData = this.ordenarHistorico(this.sistema);
    data = this.fillDataChartSet(chatData, order);
    this.sistemaChartDataSet.data = data;
    datasets.push(this.sistemaChartDataSet);
  ...
  getMaxDataSet() {
    const values = [this.sistemaChartDataSet.data]
      .join()
      .split(",")
      .map((x) => +x);
